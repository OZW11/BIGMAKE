...
def process_large_image(nos_img, model_file, target_size=256):
    # 获取输入图像的尺寸
    _, _, h, w = nos_img.shape

    # 将输入图像缩放到目标尺寸 (256x256)
    nos_img_resized = F.interpolate(nos_img, size=(target_size, target_size), mode='bilinear', align_corners=False)

    # 将缩放后的图像输入模型进行处理
    with torch.no_grad():
        output_resized = model_file(nos_img_resized)

    # 将输出图像恢复到原始尺寸
    final_output = F.interpolate(output_resized, size=(h, w), mode='bilinear', align_corners=False)
...
def test(args, data_loader, save_test_dir, device, save=False, model_file=None, loss_f=None):
    ...
    with torch.no_grad():
        for idx, (ori_img, nos_img, img_name) in enumerate(data_loader):
            ...
            # 模型去噪
            output = process_large_image(nos_img, model_file)
...
if __name__ == '__main__':
    ...
    # 初始化模型
    _model = model.FilterNet(args).to(device)
