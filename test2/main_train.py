...
parser.add_argument(
    "--train_data", default="data\Train400", type=str, help="path of train data"
)
parser.add_argument("--epoch", default=18000, type=int, help="number of train epochs")
parser.add_argument(
    "--lr", default=0.1, type=float, help="initial learning rate for Adam"
)
parser.add_argument(
    "--model_dir", default="models/DnCNN_oct", help="directory of the model"
)
...
class DnCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self._3out(x2)
        x4 = torch.nn.functional.interpolate(x3, size=(64, 64), mode="nearest")
        x5 = self._4Out(x4)
        x5 = x5 - x1

        x6 = torch.nn.functional.interpolate(x5, size=(128, 128), mode="nearest")
        x7 = self._5Out(x6)
        return y - x7
...
# 加载图像函数，调整所有图像大小
def load_images(train_data_dir, target_size=(128, 128)):
    original_images = []
    noisy_images = []
    ...
    for filename in os.listdir(train_data_dir):
        ...
        if 1:
            ...
            noisy_image = add_blocky_speckle_noise(
                image, block_size=2, noise_intensity=0.1
            )
...
if __name__ == "__main__":
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        DLoader = DataLoader(
            dataset=list(zip(noisy_images, original_images)),
        ...
        # 输出每个 epoch 的损失
        print(
            f"Epoch {epoch + 1}, Loss: {epoch_loss / len(DLoader):.4f}, Time: {elapsed_time:.2f}s"
        )
        if (epoch + 1) % 30 == 0:
            torch.save(model, os.path.join(save_dir, "final_model.pth"))
            print(
                f"Epoch {epoch + 1}训练结束，模型已保存至 {os.path.join(save_dir, 'final_model.pth')}"
            )
    # 训练结束后保存模型
    torch.save(model, os.path.join(save_dir, "final_model.pth"))
